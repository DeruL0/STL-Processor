...
void HE_MeshData::DenoiseHelper(float T) {
	...
	for (std::uint32_t i = 0; i < HE_Triangles.size(); i++){
		...
		*/
		...
		for (int j = 0; j < neighbors.size(); j++){
			float temp = DotVector(HE_Triangles[i]->Normal, neighbors[j]->Normal);
	...
	for (std::int32_t i = 0; i < HE_Vertexes.size(); i++){
		...
		for (int j = 0; j < neighbors.size(); j++){
			...
			float weight = DotVector(expect_normals[neighbors[j]->Index], diff);
	...
	for (int i = 0; i < 2; i++) {
		...
		for (int j = 0; j < 3; j++) {
			...
			if (temp != vertex && temp != neighbor) {
				...
				if (fairing == TAN) {
					if (i == 0) { a = tan(AngleBetweenVectors(diff0, diff1) / 2); }
					else		{ b = tan(AngleBetweenVectors(diff0, diff1) / 2); }
				}
				else if (fairing == COT) {
					if (i == 0) { a = 1 / tan(AngleBetweenVectors(diff3, diff2)); }
					else		{ b = 1 / tan(AngleBetweenVectors(diff3, diff2)); }
				}
				break;
			}
		}
